...
#
        ...
        if len(
            ...
            #  категории
            categ_dx = unidecode(response.xpath('//div[@id="container"]//div[@id="center"]/p/a/text()').extract()[::-1][0]).replace('.','')
        ...
        # номера цветов  картинки   list(map(lambda x: response.url[:23]+x, sd))
            ...
            with open('oc_option_value.csv', 'a', newline='') as csvfile:
                ...
                for x in range(0,len(link_img)):
                    fj=link_img[x].replace('/images/items/','')
                    spamwriter.writerow([1,2,'catalog/'+categ_dx + fj[fj.find('/'):],0,'','http://terrakot-yarn.ru'+ link_img[x],'', 1,1,2, name_img[x]])
                    spamwriter.writerow(['','','','','','','', 1,2,2, name_img[x]])

            #with open('oc_option_value_description.csv', 'a', newline='') as csvfile:
            #    spamwriter = csv.writer(csvfile, delimiter=';',
            #                            quotechar='"', quoting=csv.QUOTE_MINIMAL)
#
#
            #    for x in range(0,len(link_img)):
            #        spamwriter.writerow([1,1,2, name_img[x]])
            #        spamwriter.writerow([1,2,2, name_img[x]])
 #
