...
class JeansSpider(CrawlSpider):
    ...
    def parse_item(responsef, response):
        ...
        if (name_dx == 'None') or (len(response.xpath('//div[@id="center"]//h5[contains(text(),"Карта цветов")]').extract()) < 1):  # ищет на странице фразу "карта цветов", чтобы понять, что находимся на странице товара
            ...
            list_optoins_value = response.xpath('//div[@id="yarnDescription"]/p/text()').extract()

            for t, j in enumerate(list_options):  #for m in resp_foto:
                if str(t).strip() != '':
                    i['list_options' + str(t)] = j.strip()
                    i['list_optoins_value' + str(t)] = list_optoins_value[t].strip()
            for x in range(9, len(list_options), -1):
                i['list_options' + str(x)] = '' 
                i['list_optoins_value' + str(x)] = ''

            # номера цветов  картинки
            response.xpath('//div[@id="center"]/table[@class="main"]//tr/td[@class="items_div"]/img[re:match(@src,"jpg")]/@src').extract() #список картинок
            ...
            i['country_proizvodit'] = response.xpath('//div[@id="center"]//p[@class="status"]/a/text()').extract()[-1]
